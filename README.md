...
* [`src/cljs/cthulu_client/`](src/cljs/cthulu_client/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljs/cthulu_client/core.cljs): contains the SPA entry point, `init`
...
7. Clone this repo and open a terminal in the `cthulu-client` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/cthulu_client/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns cthulu-client.example
  (:require [cthulu-client.config :as config])
